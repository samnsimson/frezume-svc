...
def get_session_from_request(request: Request) -> Session:
    if not hasattr(request.state, 'session'): raise HTTPException(status_code=401, detail="Unauthorized")
    return request.state.session


def get_user_from_request(request: Request) -> User:
    if not hasattr(request.state, 'user'): raise HTTPException(status_code=401, detail="Unauthorized")
