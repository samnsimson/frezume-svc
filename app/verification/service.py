...
from app.lib.constants import (
    ERROR_INVALID_TOKEN,
    ERROR_TOKEN_DOES_NOT_BELONG_TO_USER,
...
class VerificationService:
    ...
    def _validate_verification(self, verification: Verification, user_id: UUID, identifier: str) -> None:
        if not verification: raise HTTPException(status_code=401, detail=ERROR_INVALID_TOKEN)
        if verification.user_id != user_id: raise HTTPException(status_code=401, detail=ERROR_TOKEN_DOES_NOT_BELONG_TO_USER)
        if verification.identifier != identifier: raise HTTPException(status_code=401, detail=ERROR_INVALID_IDENTIFIER)
        if verification.expires_at < datetime.now(timezone.utc): raise HTTPException(status_code=401, detail=ERROR_TOKEN_EXPIRED)
